...
/**
 * Beranda perpustakaan — sama persis untuk portal siswa dan portal guru.
 * Endpoint-nya pun satu (/library); yang berbeda hanya `base`, awalan
 * tautan menuju katalog dan dasbor portal yang sedang dipakai.
 */
export function LibraryView({ base, data }: { base: string; data: ApiLibrary }) {
  const {
    categories,
      ...
      <PageHead
        ...
        action={
          <div className="flex shrink-0 flex-wrap gap-3">
            <Link
              href={`${base}/perpustakaan/katalog`}
      ...
      <StaggerGroup className="grid gap-4 sm:grid-cols-3">
        {categories.map((c) => (
          <StaggerItem key={c.name}>
            <Link
              href={`${base}/perpustakaan/katalog?kategori=${encodeURIComponent(c.name)}`}
